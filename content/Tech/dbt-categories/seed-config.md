...
- **CSVファイルをBigQueryテーブルとして読み込む方法**
- **column_types、quote_columns、delimiterの設定**
- **参照データと小規模ディメンションテーブルの管理**
- **Seeds使用のベストプラクティスと制限事項**

# はじめに

dbt seedsは、CSVなどのフラットファイルをデータベーステーブルとして読み込む機能です。参照データや小規模なディメンションテーブルの管理に適しています。本記事では、seedsの設定と動作を検証し、実務で使える管理方法を明らかにします。
