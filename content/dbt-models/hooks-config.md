...
- **pre-hook、post-hook、on-run-start、on-run-endの使い分け**
- **ログ記録、権限設定、データ品質チェックの自動化**
- **フックの実行タイミングとスコープ**
- **実務で使えるフック実装パターン**

# はじめに

dbtのフック（hooks）機能は、dbt実行の特定タイミングでSQLを実行する仕組みです。ログ記録、権限設定、データ品質チェックなどの自動化に使用されます。本記事では、4種類のフックを検証し、実務で使える実装パターンを明らかにします。
